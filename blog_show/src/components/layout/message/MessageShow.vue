<template>
  <div ref="messageContainer" class="message-container">
    ...
    <div ref="formContainer" class="reply-form">
      <div class="form-header">发表评论*
        <span>(将使用Cookie存放你的个人信息)</span>
      </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      if (info) {
        this.message.nickname = info.nickname;
        this.message.email = info.email;
        this.message.web = info.web;
      }
    },
    methods: {
      getMessage(page = 0) {
        this.axios.get("/message?page=" + page)
          .then(response => {
      ...
      onSubmit() {
        if (getToken()) {
          this.message.adminMessage = true;
        }
        this.$refs.form.validate((flag, object) => {
          if (flag) {
            if (!getInfo()) {
              setInfo(this.message.nickname, this.message.email, this.message.web);
            }
