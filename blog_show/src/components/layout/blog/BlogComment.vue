	...
	<div ref="commentContainer" class="comment-container">
		<div class="comment-box" v-for="(item,index) in comments" :key="item.id">
			...
			<div class="comment-header">
				...
				<span v-if="item.adminComment" class="comment-admin">博主</span>
			</div>
			...
			<div v-if="item.replyComments"
			     v-for="(reply,i) in item.replyComments" :key="reply.id"
			     class="reply-box">
				...
				<div class="reply-header">
					...
					<span v-if="reply.adminComment" class="reply-admin">博主</span>
					@ <span>{{ reply.parentComment.nickname }}</span>
				</div>
		...
		<div ref="formContainer" class="reply-form">
			<el-form ref="form" :model="comment" :rules="rules">
				<div class="form-header">发表评论*</div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onSubmit() {
                if (getToken()) {
                    this.comment.adminComment = true;
                }
                this.$refs.form.validate((flag, object) => {
                    if (flag) {
                        ...
                        this.axios.post("/blog/comments", this.comment)
                            .then(response => {
                                if (response.data.status === 200) {
                                    this.comment.content = '';
                                    this.comment.parentComment.id = -1;
                                    this.$message.success("回复成功");
                                    this.placeholder = '消灭人类暴政，世界属于三体';
                                    this.getComment();
                                    this.cancelReply();
                                }
                            })
            ...
            getComment() {
                this.axios.get("/blog/"+this.comment.blog.id+"/comments")
                    .then(response => {
                        if (response.data.status === 200) {
